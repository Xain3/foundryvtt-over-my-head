...
describe('PlaceableHandler', () => {
  ...
  describe('constructor', () => {
    it('should initialize with the provided parameters', () => {
      expect(placeableHandler).toBeInstanceOf(Handler);
  ...
  describe('setCurrent', () => {
    it('should call setter.setCurrentPlaceable with the provided placeable', () => {
      ...
      expect(mockSetter.setCurrentPlaceable).toHaveBeenCalledWith(
        mockPlaceable
      );
  ...
  describe('getAll', () => {
    it('should call getter.getAllPlaceables and update internal list when updateProperty is true', () => {
      ...
      expect(mockGetter.getAllPlaceables).toHaveBeenCalledWith(
        'testType',
        false,
        true
      );
  ...
  describe('getCorner', () => {
    it('should call getter.getCorner with the provided parameters', () => {
      ...
      expect(mockGetter.getCorner).toHaveBeenCalledWith(
        'topLeft',
        mockPlaceable
      );
  ...
  describe('getPosition', () => {
    it('should call getter.getPosition with the provided parameters', () => {
      ...
      const result = placeableHandler.getPosition(
        mockPlaceable,
        mockManager,
        'corner'
      );

      expect(mockGetter.getPosition).toHaveBeenCalledWith(
        mockPlaceable,
        mockManager,
        'corner'
      );
  ...
  describe('isOwned', () => {
    it('should call checker.isOwned with the provided token', () => {
      const mockToken = { id: 'test-token' };
      mockChecker.isOwned.mockReturnValue(true);

      const result = placeableHandler.isOwned(mockToken);

      expect(mockChecker.isOwned).toHaveBeenCalledWith(mockToken);
      expect(result).toBe(true);
    });
  });

  describe('isOwnedAndSelected', () => {
    it('should call checker.isOwnedAndSelected with the provided token', () => {
      const mockToken = { id: 'test-token' };
      mockChecker.isOwnedAndSelected.mockReturnValue(true);

      const result = placeableHandler.isOwnedAndSelected(mockToken);

      expect(mockChecker.isOwnedAndSelected).toHaveBeenCalledWith(mockToken);
      expect(result).toBe(true);
    });
  });
});
