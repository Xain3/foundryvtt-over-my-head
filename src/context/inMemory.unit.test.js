import InMemoryContextManager from './memory.js';
...
describe('InMemoryContextManager', () => {
  ...
  describe('context delegation methods', () => {
    ...
    it('should delegate createSnapshot to context', () => {
      const expectedResult = 'snapshotResult';
      const options = { test: 'options' };
      mockContext.createSnapshot.mockReturnValue(expectedResult);

      const result = manager.createSnapshot(options);

      expect(mockContext.createSnapshot).toHaveBeenCalledWith(options);
      expect(result).toBe(expectedResult);
    });

    it('should delegate createSnapshot with default options', () => {
      const expectedResult = 'snapshotResult';
      mockContext.createSnapshot.mockReturnValue(expectedResult);

      const result = manager.createSnapshot();

      expect(mockContext.createSnapshot).toHaveBeenCalledWith({});
      expect(result).toBe(expectedResult);
    });
  });
