...
jest.mock('@/helpers/moduleGetter', () => ({
  getModule: jest.fn()
}));
...
describe('RootMapParser', () => {
  ...
  describe('Input Validation', () => {
    ...
    it('should validate key parameter when provided', () => {
      PathUtils.resolvePath.mockReturnValue({ notify: jest.fn() });
    ...
    it('should parse Foundry VTT UI configuration', () => {
      ...
      PathUtils.resolvePath.mockImplementation((path) => {
        const mockObjects = {
          ...
          'game.hooks': { on: jest.fn(), off: jest.fn() }
        };
...
/**
 * @file rootMapParser.test.js
 * @description This file contains unit tests for the RootMapParser class.
 * @path src/context/helpers/rootMapParser.test.js
 */
...
jest.mock('@helpers/resolvePath.js', () => ({
  resolvePath: jest.fn()
}));
...
describe('RootMapParser', () => {
  ...
  describe('Input Validation', () => {
    it('should validate rootMap parameter', () => {
      resolvePath.mockImplementation((path) => {
        if (path === 'game.ui.notifications') return { notify: jest.fn() };
        if (path === 'game.data.nested') return { value: 'test' };
        return null;
      });
    ...
    it('should validate key parameter when provided', () => {
      resolvePath.mockReturnValue({ notify: jest.fn() });
  ...
  describe('Successful Cases', () => {
    ...
    it('should parse specific key from root map', () => {
      ...
      expect(result).toEqual({ notify: expect.any(Function) });
      expect(resolvePath).toHaveBeenCalledWith('game.ui.notifications', {
        namespace: mockNamespace,
        module: 'test-module-id'
      });
    });
    ...
    it('should handle null values in root map', () => {
      const rootMapWithNull = { element: null, other: 'game.ui' };
      resolvePath.mockReturnValue({ ui: true });
    ...
    it('should parse nested objects recursively', () => {
      ...
      resolvePath.mockReturnValue({ nested: true });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle deeply nested objects', () => {
      ...
      resolvePath.mockReturnValue({ deep: 'value' });
    ...
    it('should handle empty string paths', () => {
      resolvePath.mockReturnValue(mockNamespace);
    ...
    it('should handle special characters in keys', () => {
      ...
      resolvePath.mockImplementation((path) => ({ resolved: path }));
  ...
  describe('Constants Integration', () => {
    it('should use manifest ID as default module', () => {
      resolvePath.mockReturnValue({ test: true });
      ...
      expect(resolvePath).toHaveBeenCalledWith('game.test', {
        namespace: mockNamespace,
        module: 'test-module-id'
      });
    });
  ...
  describe('Alternative Constants Configuration', () => {
    it('should work with different manifest configurations', () => {
      ...
      resolvePath.mockReturnValue({ alternative: true });
      ...
      expect(resolvePath).toHaveBeenCalledWith('game.alternative', {
        namespace: mockNamespace,
        module: 'alternative-module'
      });
    ...
    it('should override default module with provided module parameter', () => {
      resolvePath.mockReturnValue({ custom: true });
      ...
      expect(resolvePath).toHaveBeenCalledWith('game.custom', {
        namespace: mockNamespace,
        module: 'custom-module-override'
      });
    });
  ...
  describe('Real-world Scenarios', () => {
    ...
    it('should parse complex nested configuration', () => {
      ...
      resolvePath.mockImplementation((path) => ({ mocked: path }));
    ...
    it('should handle circular reference detection', () => {
      ...
      resolvePath.mockReturnValue({ circular: true });
