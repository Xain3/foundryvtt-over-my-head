/**
 * @file externalCtxRootManager.js
 * @description This file contains the RootManager class for managing external context roots.
 * @path /src/context/helpers/externalCtxRootManager.js
 */
...
/**
 * @class RootManager
 * @description Manages external context roots by determining, setting, and retrieving root objects
 * based on source identifiers and configuration data. External roots can be objects in global scope,
 * specific objects in modules, or external storage sources.
 */
class RootManager {
  /**
   * Creates a new RootManager instance.
   * @param {Object} options - The configuration options.
   * @param {Object} options.rootMap - The root map object containing source identifier mappings.
   * @param {string} [options.rootIdentifier] - The identifier for the initial root.
   */
  constructor({rootMap, rootIdentifier = undefined}) {
    this.rootMap = rootMap;
    this.rootIdentifier = rootIdentifier;
    this.root = this._determineRoot(rootIdentifier);
  }
  ...
  _determineRoot(sourceString, throwError = true, logError = true) {
    ...
    try {
      ...
      const rootMapEntry = this.rootMap?.rootMap?.[sourceString];
      if (rootMapEntry === undefined) {
        const errorMsg = `Could not determine remote context root. Source string '${sourceString}' is not a valid key in the root map`;
        if (throwError) {
