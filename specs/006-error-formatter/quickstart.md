...
2. **Call it with a simple error** to verify the module prefix is added:
   ...
   console.log(output); // e.g., "Over My Head: Configuration failed"
   ```
...
3. **Enable optional contexts** by passing options:
   ...
   When `includeStack` is `true`, the stack prints up to 20 lines and adds `[Full trace: /tmp/omh-error-XXX.log]` if truncated.
