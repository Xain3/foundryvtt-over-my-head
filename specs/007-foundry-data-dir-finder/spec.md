...
4. **Given** FoundryVTT is not installed, **When** the finder utility runs, **Then** it returns an empty result with `found: false`
...
### User Story 3 - Diagnostic and Verbose Reporting (Priority: P3)

A developer troubleshoots why their deployment script cannot find the FoundryVTT installation. They need detailed information about which paths were checked and why the search failed.
...
1. **Given** verbose mode is enabled, **When** the finder executes, **Then** it logs each path as it's checked with platform and user information
2. **Given** no installation is found and verbose mode is enabled, **When** the finder completes, **Then** it reports all paths that were checked and confirms none were valid
3. **Given** an installation is found with verbose mode, **When** the finder completes, **Then** it logs the successful path and confirms it's a valid directory
...
3. **Given** the finder needs to return potential paths without checking existence, **When** `getPaths()` is called, **Then** it returns all platform-appropriate paths without filesystem access
...
6. **Priority Order**: When multiple potential paths exist, returns the first valid path found (following platform-specific priority)
