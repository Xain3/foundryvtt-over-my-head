...
vi.mock('#helpers/settingsRetriever.mjs', async () => {
  return vi.importActual('#helpers/settingsRetriever.mjs');
});
...
describe('SettingsHandler', () => {
  ...
  describe('constructor', () => {
    it('should initialize properly with config containing debugMode', () => {
      ...
      expect(handler.parsedSettings).toBe(
        mockConfig.constants.settings.settingsList
      );
      expect(SettingsParser).toHaveBeenCalledWith({
        config: mockConfig,
        utils: mockUtils,
        context: mockContext,
      });
      expect(SettingsRegistrar).toHaveBeenCalledWith({
        config: mockConfig,
        utils: mockUtils,
        context: mockContext,
      });
  ...
  describe('registerDebugModeSetting', () => {
    ...
    it('should return failure result when debugMode setting does not exist', () => {
      ...
      expect(result.errorMessages).toEqual([
        'Debug mode setting not found in parsed settings',
      ]);
  ...
  describe('Generic setting methods', () => {
    describe('registerSettingByKey', () => {
      ...
      it('should return failure result when setting does not exist', () => {
        ...
        expect(result.successCounter).toBe(0);
        expect(result.errorMessages).toEqual([
          "Setting with key 'nonExistentSetting' not found in parsed settings",
        ]);
        expect(result.message).toBe(
          "Setting with key 'nonExistentSetting' not found in parsed settings"
        );
